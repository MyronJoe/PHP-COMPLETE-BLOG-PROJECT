<?php
require_once(ROOT_PATH . "/app/controllers/topics.php");
$topics = selectAll('topics');
?>
<!-- <form action="#" class="nav_form desktop_nav">
    <input type="search" class="search-data" placeholder="Search" required>
    <button type="submit" class="fas fa-search"></button>
</form> -->

<form action="search.php" method="post" class="nav_form desktop_nav">
    <input type="search" class="search-data" placeholder="Search..." name="search-term">
    <button type="submit" class="fas fa-search"></button>
</form>


<div class="top_stories">
    <div class="header_line">
        <div class="header_title_side">
            <p><span>Top Stories</span></p>
        </div>
    </div>
    <div class="mobile_card">
        <div class="mobile_img_sec">

            <div class="mtopic">
                <p style="font-size: 13px;">1</p>
            </div>
            <a href="#">
                <img src="assets/images/IMG-20220519-WA0023.jpg" alt="NEWS">
            </a>
        </div>
        <div class="mobile_title_sec">
            <a href="#">
                <h4 class="title">This is the title of this post bcoz it is very important</h4>
            </a>

            <small>Jul 2, 2022 | 13:22</small>
        </div>
    </div>

    <div class="mobile_card">
        <div class="mobile_img_sec">

            <div class="mtopic">
                <p style="font-size: 13px;">2</p>
            </div>
            <a href="#">
                <img src="assets/images/images(56).jpg" alt="NEWS">
            </a>
        </div>
        <div class="mobile_title_sec">
            <a href="#">
                <h4 class="title" style="margin-bottom: 5px;">This is the title of this post</h4>
            </a>

            <small>Jul 2, 2022 | 13:22</small>
        </div>
    </div>
    <div class="mobile_card">
        <div class="mobile_img_sec">

            <div class="mtopic">
                <p style="font-size: 13px;">3</p>
            </div>
            <a href="#">
                <img src="assets/images/IMG-20220519-WA0023.jpg" alt="NEWS">
            </a>
        </div>
        <div class="mobile_title_sec">
            <a href="#">
                <h4 class="title" style="margin-bottom: 5px;">This is the title of this post</h4>
            </a>

            <small>Jul 2, 2022 | 13:22</small>
        </div>
    </div>
    <div class="mobile_card">
        <div class="mobile_img_sec">

            <div class="mtopic">
                <p style="font-size: 13px;">4</p>
            </div>
            <a href="#">
                <img src="assets/images/NACOMES.jpeg" alt="NEWS">
            </a>
        </div>
        <div class="mobile_title_sec">
            <a href="#">
                <h4 class="title" style="margin-bottom: 5px;">This is the title of this post</h4>
            </a>

            <small>Jul 2, 2022 | 13:22</small>
        </div>
    </div>
    <div class="mobile_card">
        <div class="mobile_img_sec">

            <div class="mtopic">
                <p style="font-size: 13px;">5</p>
            </div>
            <a href="#">
                <img src="assets/images/IMG-20220519-WA0023.jpg" alt="NEWS">
            </a>
        </div>
        <div class="mobile_title_sec">
            <a href="#">
                <h4 class="title" style="margin-bottom: 5px;">This is the title of this post</h4>
            </a>

            <small>Jul 2, 2022 | 13:22</small>
        </div>
    </div>
</div>

<div class="youtube" style="margin-bottom:10px;">
    <iframe width="100%" height="270" src="https://www.youtube.com/embed/byBI_GmWTCM" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>
</div>


<div class="Comming soon">
    <div class="header_line">
        <div class="header_title_side">
            <p><span>Coming Soon</span></p>
        </div>
    </div>
    <div class="mobile_card">
        <div class="mobile_img_sec">

            <div class="mtopic">
                <p style="font-size: 13px;">1</p>
            </div>
            <a href="#">
                <img src="assets/images/IMG-20220519-WA0023.jpg" alt="NEWS">
            </a>
        </div>
        <div class="mobile_title_sec">
            <a href="#">
                <h4 class="title">This is the title of this post bcoz it is very important</h4>
            </a>

            <small>Jul 2, 2022 | 13:22</small>
        </div>
    </div>

    <div class="mobile_card">
        <div class="mobile_img_sec">

            <div class="mtopic">
                <p style="font-size: 13px;">2</p>
            </div>
            <a href="#">
                <img src="assets/images/images(56).jpg" alt="NEWS">
            </a>
        </div>
        <div class="mobile_title_sec">
            <a href="#">
                <h4 class="title" style="margin-bottom: 5px;">This is the title of this post</h4>
            </a>

            <small>Jul 2, 2022 | 13:22</small>
        </div>
    </div>
    <div class="mobile_card">
        <div class="mobile_img_sec">

            <div class="mtopic">
                <p style="font-size: 13px;">3</p>
            </div>
            <a href="#">
                <img src="assets/images/IMG-20220519-WA0023.jpg" alt="NEWS">
            </a>
        </div>
        <div class="mobile_title_sec">
            <a href="#">
                <h4 class="title" style="margin-bottom: 5px;">This is the title of this post</h4>
            </a>

            <small>Jul 2, 2022 | 13:22</small>
        </div>
    </div>
    <div class="mobile_card">
        <div class="mobile_img_sec">

            <div class="mtopic">
                <p style="font-size: 13px;">4</p>
            </div>
            <a href="#">
                <img src="assets/images/NACOMES.jpeg" alt="NEWS">
            </a>
        </div>
        <div class="mobile_title_sec">
            <a href="#">
                <h4 class="title" style="margin-bottom: 5px;">This is the title of this post</h4>
            </a>

            <small>Jul 2, 2022 | 13:22</small>
        </div>
    </div>
    <div class="mobile_card">
        <div class="mobile_img_sec">

            <div class="mtopic">
                <p style="font-size: 13px;">5</p>
            </div>
            <a href="#">
                <img src="assets/images/IMG-20220519-WA0023.jpg" alt="NEWS">
            </a>
        </div>
        <div class="mobile_title_sec">
            <a href="#">
                <h4 class="title" style="margin-bottom: 5px;">This is the title of this post</h4>
            </a>

            <small>Jul 2, 2022 | 13:22</small>
        </div>
    </div>
</div>

<div class="newslater">
    <h3 class="mb">Newsletter</h3>
    <p>Get weekly updates and entertainment news around Africa. Don't worry, we don't spam.</p>
    <form action="#" class="nav_form mt">
        <input type="search" class="search-data" placeholder="Search" required>
        <button type="submit" class="fas fa-search"></button>
    </form>
</div>


<div >
    <ul class="tags"> 
    <?php foreach ($topics as $key => $topic): ?>
        <li class="tag"><a href="<?php echo BASE_URL . '/category.php?t_id=' . $topic['id'] . '&name=' . $topic['name']?>"><?php echo $topic['name'] ?></a></li>
    <?php endforeach ?>
    </ul>
</div>
